...
namespace conveni
{
    public partial class Goods_ADD : Form
    {
        ...
        private void Accept_Click(object sender, EventArgs e)
        {
            ...
            if (ID.Text == "")
            {
                MessageBox.Show("ID는 필수로 입력해야 합니다.");
            }
            else if (Amount.Text == "")
            {
                MessageBox.Show("수량은 필수로 입력해야 합니다.");
            }
            else
            {
                ...
                if (flag != 0)
                {
                    ...
                    name = Global.Goods_list[flag].name;
                    price = Convert.ToInt32(Global.Goods_list[flag].price);
                }
                else
                {
                    amount = Convert.ToInt32(Amount.Text);//입력 받기
                ...
                else
                {
                    ...
                    goods.amount = amount;
                    goods.name = name;
                    goods.price = Global.Goods_list[flag].price;
